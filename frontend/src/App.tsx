...
//       const user = (userData as Record<string, { score: number; accuracy: number }>)[userId]
...
// Sample user data containing scores and accuracy
const userData: Record<string, { score: number; accuracy: number }> = {
  user1: { score: 75, accuracy: 0.8 },
  user2: { score: 65, accuracy: 0.7 },
  user3: { score: 85, accuracy: 0.9 },
  student001: { score: 70, accuracy: 0.75 },
...
const App: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (userId in userData) {
      ...
      setPrediction({
        predicted_rank: predictedRank,
        insights: {
          weak_areas: weakAreas[userId] || [], // Ensures no undefined error
          improvement_trend: user.score > 70 ? "Improving" : "Needs improvement",
          overall_accuracy: user.accuracy,
        },
      });
  ...
  return (
    <div className="flex flex-grow flex-col w-full h-full items-center justify-center bg-gray-100 p-6">
      ...
      <Card className="w-full max-w-md text-center">
        {/* Card Header with Title */}
        <CardHeader className="bg-blue-600 text-white text-3xl font-bold text-center py-4 rounded-t-lg">
          <CardTitle>Student Rank Predictor</CardTitle>
        </CardHeader>
        ...
        <CardContent>
          ...
          <form onSubmit={handleSubmit} className="space-y-2 bg-slate-400">
            <div className="flex-row">
              <Label htmlFor="userId" className="font-bold text-xl">
                Enter User ID
              </Label>
              <Input
                id="userId"
              ...
              <p className="text-sm text-gray-400">
                Sample IDs: <b>user1, user2, user3, student001, student002</b>
              </p>
          ...
          {prediction && (
            <div className="mt-6 p-5 bg-gray-50 rounded-lg shadow-md">
              <h2 className="text-lg font-semibold">Prediction Results</h2>
              <p>
                <b>Predicted Rank:</b> {prediction.predicted_rank}
              </p>

              {/* Insights Section */}
              <h3 className="text-md font-semibold mt-4">Insights</h3>
              <p>
                <b>❌ Weak Areas:</b> {prediction.insights.weak_areas.join(", ")}
              </p>
              <p>
                <b>📈 Improvement Trend:</b> {prediction.insights.improvement_trend}
              </p>
              <p>
                <b>🎯 Overall Accuracy:</b>{" "}
                {(prediction.insights.overall_accuracy * 100).toFixed(2)}%
              </p>
      ...
      {/* Footer Section */}
      <footer className="text-center text-cyan-950 p-4 mt-8">
        © {new Date().getFullYear()} Kartika Kannojiya. All rights reserved.
      </footer>
    </div>
  );
};
